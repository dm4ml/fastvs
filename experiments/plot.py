...
from tests.utils import numpy_knn, create_dataset, scipy_knn
...
for dim in dimensions:
    for _ in range(num_trials):
        ...
        numpy_time, _ = execute_with_timer(numpy_knn, data, query_point, k, metric)
        numpy_qps = 1 / numpy_time

        # Time SciPy implementation
        scipy_time, _ = execute_with_timer(scipy_knn, data, query_point, k, metric)

        scipy_qps = 1 / scipy_time
        ...
        results_dims.append(
            {"Dimension": dim, "Implementation": "NumPy", "QPS": numpy_qps}
        )
        results_dims.append(
            {"Dimension": dim, "Implementation": "SciPy", "QPS": scipy_qps}
        )

# Convert results to DataFrame and calculate average QPS for each dimension and implementation
df_dims = pd.DataFrame(results_dims)


def drop_extreme_qps(group):
    lowest_qps = group["QPS"].nsmallest(1)
    highest_qps = group["QPS"].nlargest(1)
    return group.drop(lowest_qps.index).drop(highest_qps.index)


# Drop the lowest and highest QPS values for each implementation and dimension
df_dims = (
    df_dims.groupby(["Dimension", "Implementation"])
    .apply(drop_extreme_qps)
